const API_BASE = window.BIDDING_ASSISTANT_API || `${window.location.origin}/api/bidding`
...
async function openSourceSnippet(tab, item) {
  ...
  try {
    const resp = await fetch(`${API_BASE}/jobs/${state.jobId}/source?${params.toString()}`, { headers: mergeHeaders() })
    if (!resp.ok) {
...
async function pollJob(jobId) {
  pollTimer = setTimeout(async () => {
    try {
      const resp = await fetch(`${API_BASE}/jobs/${jobId}`, { headers: mergeHeaders() })
